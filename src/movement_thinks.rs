...
pub fn insert_narazu_src_by_ms_km(
    ...
    for i_dir in 0..KM_UGOKI_LN {
        ...
        match *p_kmdir {
            ...
            N(slider) => insert_narazu_n(gen_ky, result, slider, dx, dy, km_dst), // 北
            NNW => insert_narazu_nnw(gen_ky, result, dx, dy, km_dst),             // 北北西
            NW(slider) => insert_narazu_nw(gen_ky, result, slider, dx, dy, km_dst), // 北西
            W(slider) => insert_narazu_w(gen_ky, result, slider, dx, dy, km_dst), // 西
            SW(slider) => insert_narazu_sw(gen_ky, result, slider, dx, dy, km_dst), // 南西
            SSW => insert_narazu_ssw(gen_ky, result, dx, dy, km_dst),             // 南南西
            S(slider) => insert_narazu_s(gen_ky, result, slider, dx, dy, km_dst), // 南
            SSE => insert_narazu_sse(gen_ky, result, dx, dy, km_dst),             // 南南東
            SE(slider) => insert_narazu_se(gen_ky, result, slider, dx, dy, km_dst), // 南東
            Num => break,
        }
    }
}
...
/// cyclomatic complexity を避けるために分けただけ。
fn insert_narazu_nnw(gen_ky: &Position, result: &mut HashSet<umasu>, dx: i8, dy: i8, km_dst: Koma) {
...
/// cyclomatic complexity を避けるために分けただけ。
fn insert_narazu_ssw(gen_ky: &Position, result: &mut HashSet<umasu>, dx: i8, dy: i8, km_dst: Koma) {
...
/// cyclomatic complexity を避けるために分けただけ。
fn insert_narazu_sse(gen_ky: &Position, result: &mut HashSet<umasu>, dx: i8, dy: i8, km_dst: Koma) {
...
pub fn insert_narumae_src_by_ms_km(
    ...
    for i_dir in 0..KM_UGOKI_LN {
        ...
        match *p_kmdir {
            ...
            N(slider) => insert_narumae_n(gen_ky, result, slider, dx, dy, km_src), // 北
            NNW => insert_narumae_nnw(gen_ky, result, dx, dy, km_src),             // 北北西
            NW(slider) => insert_narumae_nw(gen_ky, result, slider, dx, dy, km_src), // 北西
            W(slider) => insert_narumae_w(gen_ky, result, slider, dx, dy, km_src), // 西
            SW(slider) => insert_narumae_sw(gen_ky, result, slider, dx, dy, km_src), // 南西
            SSW => insert_narumae_ssw(gen_ky, result, dx, dy, km_src),             // 南南西
            S(slider) => insert_narumae_s(gen_ky, result, slider, dx, dy, km_src), // 南
            SSE => insert_narumae_sse(gen_ky, result, dx, dy, km_src),             // 南南東
            SE(slider) => insert_narumae_se(gen_ky, result, slider, dx, dy, km_src), // 南東
            Num => break,
        }
    }
}
 ...
 * 王が取られる局面を除く手を選ぶぜ☆（＾～＾）
 */
pub fn filtering_ss_except_oute(searcher: &Searcher, ss_hashset_input: &mut HashSet<u64>) {
    // 自玉の位置
 ...
 * 千日手には喜んで飛び込めだぜ☆（＾▽＾）ｗｗｗ
 */
pub fn filtering_ss_except_jisatusyu(searcher: &mut Searcher, ss_hashset_input: &mut HashSet<u64>) {
    // 残すのはここに退避する☆（＾～＾）
    let mut ss_hashset_pickup: HashSet<u64> = HashSet::new();
 ...
 */
pub fn filtering_ss_except_sennitite(searcher: &mut Searcher, ss_hashset_input: &mut HashSet<u64>) {
