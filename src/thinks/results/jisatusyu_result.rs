...
/// TODO 利きを再計算したい
pub fn is_jisatusyu(ss:&Sasite)->bool{
    // 移動元升、動かした駒の先後、駒種類、
    let km_src = UCHU_WRAP.read().unwrap().ky.get_km_by_ms( ss.src );
    let (sn_teban,_kms) = km_to_sn_kms( &km_src );
    // 相手番の先後
    let sn_aite = hanten_sn( &sn_teban );

    // 升の利き数だが、指した後で再計算が要るはず
    let kikisu = UCHU_WRAP.read().unwrap().kiki_su_by_sn[ sn_to_num( &sn_aite) ].get_su_by_ms( ss.dst );
    let result = 0<kikisu;
    // g_writeln(&format!(
