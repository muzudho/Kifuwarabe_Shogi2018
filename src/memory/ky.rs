...
impl Kyokumen{
     ...
     * return : 取った駒
     */
    pub fn do_sasite(&mut self, sn:&Sengo, ss:&Movement) -> Koma {
        ...
        if ss.source==SS_SRC_DA {
            ...
            if ss.promotion {
                // 成りなら
                km = km_to_prokm( &self.get_km_by_ms(ss.source) );
            } else {
                km = self.get_km_by_ms(ss.source);
            }
            self.set_km_by_ms(ss.source, Koma::Kara);
        }

        // 移動先升に駒があるかどうか
        if let Koma::Kara=self.get_km_by_ms(ss.destination) {
            cap = Koma::Kara;
        } else {
            // 移動先升の駒を盤上から消し、自分の持ち駒に増やす
            cap = self.get_km_by_ms(ss.destination);
            let mg = km_to_mg(cap);
            self.add_mg(mg,1);
        }

        // 移動先升に駒を置く
        self.set_km_by_ms(ss.destination, km);
     ...
     * 手目のカウントが増えたりはしないぜ☆（＾～＾）
     */
    pub fn undo_sasite(&mut self, sn:&Sengo, ss:&Movement, cap:&Koma){
        ...
        if ss.source==SS_SRC_DA {
            ...
            if ss.promotion {
                // 成ったなら、成る前へ
                km = prokm_to_km( &self.get_km_by_ms(ss.destination) );
            } else {
                km = self.get_km_by_ms(ss.destination);
            }
        }

        // 移動先の駒を、取った駒（あるいは空）に戻す
        self.set_km_by_ms(ss.destination, *cap);
        match *cap {
            Koma::Kara =>{},
            _ => { 
                // 自分の持ち駒を減らす
        ...
        // 移動元升に、動かした駒を置く
        self.set_km_by_ms(ss.source, km);
    }
